...
func (a *Agent) run() (err error) {
	...
	log.Infof("run subscribe agent")

	a.osSignalCtrl()

	a.m = module.New(types.Namespace, module.WithPort(31005))

	log.Infof("use port (%d)", a.m.Port())

	if err = a.m.Bind(types.SubscribeFunctionType, a.subscribeFunction); err != nil {
		return
	}
